...
func TestBundleResolverCache(t *testing.T) {
	...
	repo := getRegistryServiceIP(ctx, t, c, namespace) + ":5000/cachetest-" + task.Name
	...
	// THEN
	assertCacheAnnotations(t, actualTr1, expectedStoreAnnotations)
	assertCacheAnnotations(t, actualTr2, expectedRetrieveAnnotations)
}
...
func assertCacheAnnotations(t *testing.T, tr *v1.TaskRun, expected map[string]string) {
	t.Helper()

	actual := tr.GetAnnotations()
	asJSON, _ := json.MarshalIndent(actual, "", "  ")
	t.Logf("Annotations of %s: \n%s", tr.Name, asJSON)
