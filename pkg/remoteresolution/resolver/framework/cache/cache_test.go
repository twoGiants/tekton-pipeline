...
//   - cache miss with failed resolution does not block retry
...
func TestGetCachedOrResolveFromRemote(t *testing.T) {
	...
	t.Run("cache miss and resolution error does not block retry", func(t *testing.T) {
		// GIVEN
		expectedError := errors.New("resolution failed")
		resolveErrFn := func() (resolutionframework.ResolvedResource, error) {
			return nil, expectedError
		}
		resolveSuccessFn := func() (resolutionframework.ResolvedResource, error) {
			return &mockResolvedResource{data: []byte("recovered data")}, nil
		}
		cache := newResolverCache(100, 1*time.Hour)
		defer cache.Clear()
		...
		if !errors.Is(firstTryErr, expectedError) {
			t.Fatalf("expected error %v, got error %v", expectedError, firstTryErr)
		}

		if retryErr != nil {
			t.Fatalf("expected retry to succeed after failed resolution, got error: %v", retryErr)
		}
