...
func (c *Reconciler) resolvePipelineState(
	...
	for _, pipelineTask := range pipelineTasks {
		...
		getChildPipelineRunFunc := func(name string) (*v1.PipelineRun, error) {
			return c.pipelineRunLister.PipelineRuns(pr.Name).Get(name)
		}
...
func (c *Reconciler) createChildPipelineRun(
	...
	newChildPipelineRun := &v1.PipelineRun{
		ObjectMeta: metav1.ObjectMeta{
			...
			// TODO(stas): bad name getTaskrunAnnotations
...
// filterChildPipelineRunsForParentPipelineRunStatus returns child (PIP) PipelineRuns owned by the parent PipelineRun.
func filterChildPipelineRunsForParentPipelineRunStatus(logger *zap.SugaredLogger, pr *v1.PipelineRun, childPipelineRuns []*v1.PipelineRun) []*v1.PipelineRun {
	...
	for _, child := range childPipelineRuns {
		// Only process child (PIP) PipelineRuns that are owned by this parent PipelineRun.
...
func updatePipelineRunStatusFromChildRefs(logger *zap.SugaredLogger, pr *v1.PipelineRun, childPipelineRuns []*v1.PipelineRun, trs []*v1.TaskRun, customRuns []*v1beta1.CustomRun) {
	// If no child (PIP) PipelineRun, TaskRun or CustomRun was found, nothing to be done. We never remove child references from the status.
	...
	// Map PipelineTask names to child (PIP) PipelineRun, TaskRun or CustomRun child references that were already in the status
	...
	// Loop over all the child (PIP) PipelineRuns associated to the parent PipelineRun
	for _, fcpr := range filteredChildPipelineRuns {
		...
		logger.Infof("Found a child (PIP) PipelineRun %s that was missing from the parent PipelineRun status", fcpr.Name)
