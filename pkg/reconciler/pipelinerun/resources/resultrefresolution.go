...
func validateArrayResultsIndex(allResolvedResultRefs ResolvedResultRefs) error {
	for _, r := range allResolvedResultRefs {
		...
		if r.ResultReference.ResultsIndex == nil {
			continue
		}

		if *r.ResultReference.ResultsIndex >= len(r.Value.ArrayVal) {
			return fmt.Errorf(
				"array Result Index %d for Task %s Result %s is out of bound of size %d",
				*r.ResultReference.ResultsIndex,
				r.ResultReference.PipelineTask,
				r.ResultReference.Result,
				len(r.Value.ArrayVal),
			)
		}
	}
	return nil
