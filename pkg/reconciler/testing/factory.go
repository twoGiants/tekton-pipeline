...
// for the parent Pipeline and the expected child PipelineRuns for each child Pipeline.
func TwoPipelinesInPipelineMixedTasks(t *testing.T, namespace, parentPipelineRunName string) (*v1.Pipeline, *v1.PipelineRun, []*v1.PipelineRun) {
	t.Helper()
	uid := "bar"
  ...
  - name: %s
    pipelineSpec:
      ...
      - name: %s
        taskRef:
          name: %s
`, parentPipelineName, namespace, childPipelineName1, childPipelineTaskName1, childPipelineName2, childPipelineTaskName2, taskRefName))
...
spec:
  pipelineSpec:
    ...
    - name: %s
      taskRef:
        name: %s
`, childPipelineTaskName2, taskRefName),
	)

	return parentPipeline, parentPipelineRun, []*v1.PipelineRun{expectedChildPipelineRun1, expectedChildPipelineRun2}
}
...
func NestedPipelinesInPipeline(t *testing.T, namespace, parentPipelineRunName string) (*v1.Pipeline, *v1.PipelineRun, *v1.PipelineRun, *v1.PipelineRun) {
	...
	parentPipelineName := "parent-pipeline"
	childPipelineName := "child-pipeline"
	grandchildPipelineName := "grandchild-pipeline"
